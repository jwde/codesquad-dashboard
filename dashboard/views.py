...
def dashboard(request, type_requested=None):
    ...
    def teacher_dashboard():
        ...
        for s in students:
            ...
            profile = {'name': name, 'about': s.about_me, 'projects': s.projects, 'image': s.profile.image}
            student_profiles.append(profile)
        # students = frozenset().union(*student_sets)
        # student_names = ['{} {}'.format(s.profile.user.first_name, s.profile.user.last_name)\
        #                  for s in students]
# =======
#         courses = request.user.profile.teacher.courses;
#         if not courses:
...
# >>>>>>> Stashed changes
        return render(request, "teacher_dashboard.html",\
                      ...
                      'name': request.user.first_name}) #student_names})
